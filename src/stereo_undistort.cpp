...
/*    StereoUndistort::StereoUndistort(const ros::NodeHandle &nh,
            ...
            : nh_(nh),
              ...
              it_(nh_),
              queue_size_(getQueueSize()),
              stereo_image_sub_(it_, "raw/stereo/image", queue_size_),
              first_undistorter_ptr_(nullptr),
              second_undistorter_ptr_(nullptr),
              frame_counter_(0)
    {
...
//                    "raw/second/camera_info", queue_size_);
    ...
    void StereoUndistort::stereoImageCallback(const sensor_msgs::ImageConstPtr &image_msg_in)
    {
        ...
        // split stereo image into two images
        cv::Mat stereo_image = image_in_ptr->image;
        int width = stereo_image.cols;
        int height = stereo_image.rows;
        cv::Mat* first_img; cv::Mat* second_img;
        first_img = new cv::Mat(stereo_image, cv::Rect(0, 0, width / 2, height));
        second_img = new cv::Mat(stereo_image, cv::Rect(width / 2, 0, width / 2, height));

        // process first image
        updateUndistorter(CameraSide::FIRST);
        image_out_ptr->header.frame_id = first_output_frame_;
        first_undistorter_ptr_->undistortImage(*first_img,&(image_out_ptr->image));
        first_image_pub_.publish(*(image_out_ptr->toImageMsg()));

        // process second image
        updateUndistorter(CameraSide::SECOND);
        image_out_ptr->header.frame_id = second_output_frame_;
        second_undistorter_ptr_->undistortImage(*second_img,&(image_out_ptr->image));
        second_image_pub_.publish(*(image_out_ptr->toImageMsg()));

        // publish camera info
        std_msgs::Header header = image_msg_in->header;
        header.frame_id = first_output_frame_;
        sendCameraInfo(header, CameraSide::FIRST, CameraIO::OUTPUT);

        header = image_msg_in->header;
